...
void Init(sf::Window& window, sf::RenderTarget& target, bool loadDefaultFont)
{
    ...
    const auto targetSize = static_cast<sf::Vector2f>(target.getSize());
    // init rendering
    io.DisplaySize = ImVec2(targetSize.x, targetSize.y);
...
void Image(const sf::Texture& texture, const sf::Vector2f& size,
    const sf::Color& tintColor, const sf::Color& borderColor)
{
    const ImVec2 imSize = ImVec2(size.x, size.y);
    const ImColor imTintColor = ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a);
    const ImColor imBorderColor = ImColor(borderColor.r, borderColor.g, borderColor.b, borderColor.a);

    ImGui::Image((void*)texture.getNativeHandle(), imSize, ImVec2(0, 0), ImVec2(1, 1), imTintColor, imBorderColor);
}
    ...
    const sf::Color& tintColor, const sf::Color& borderColor)
{
    ...
    const ImColor imTintColor = ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a);
    const ImColor imBorderColor = ImColor(borderColor.r, borderColor.g, borderColor.b, borderColor.a);
    ImGui::Image((void*)texture.getNativeHandle(), imSize, uv0, uv1, imTintColor, imBorderColor);
}
...
ImColor ColorConvertFloat4ToU32(const sf::Color& color)
{
    ...
    float thickness)
{
    ...
    sf::Vector2f pos = sf::Vector2f(cursorPos.x, cursorPos.y);
    auto linePos1 = a+pos;
    auto linePos2 = b+pos;

    draw_list->AddLine(
            ImVec2(linePos1.x, linePos1.y),
            ImVec2(linePos2.x, linePos2.y),
            ColorConvertFloat4ToU32(color), thickness);
}
                     ...
                     const sf::Color& bgColor, const sf::Color& tintColor)
{
    ...
    const ImVec2 imSize = ImVec2(size.x, size.y);
    const ImColor imTintColor = ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a);
    const ImColor imBgColor = ImColor(bgColor.r, bgColor.g, bgColor.b, bgColor.a);

    return ImGui::ImageButton((void*)texture.getNativeHandle(), imSize, uv0, uv1, framePadding, imBgColor, imTintColor);
}
